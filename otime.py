...
class Ecard():
    ...
    def get_codes_and_times(self):
        ...
        codes = []
        times = 0
        self.codes_and_times = []
        lap = []
        last_time = timedelta(0)
        while self.dump[j] > 0:
            time = (calc_time(self.dump[j + 1], self.dump[j + 2]))
            self.codes_and_times.append([self.dump[j], time, time - last_time])
    ...
    def print(self):
        ...
        print('Sluttid ' + set_time(self.codes_and_times[-1][1]))
        i = 1
        for item in self.codes_and_times:

            if i == num_ctrl + 1:
                print('Post ' + 'F ' + ':  ' + set_time(item[2]) + '  ' + set_time(item[1]) + '  ' + str(item[0]))
                break
            else:
                print('Post ' + str(i).zfill(2) + ':  ' + set_time(item[2]) + '  ' + set_time(item[1]) + '  ' + str(
                    item[0]))
            i += 1
